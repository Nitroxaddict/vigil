...
import (
	...
	wtTypes "github.com/Nitroxaddict/vigil/pkg/types"
...
var _ = Describe("the auth module", func() {
	...
	Describe("GetAuthURL", func() {
		It("should create a valid auth url object based on the challenge header supplied", func() {
			...
			imageRef, err := ref.ParseNormalizedNamed("nitroxaddict/vigil")
			Expect(err).NotTo(HaveOccurred())
			expected := &url.URL{
				Host:     "ghcr.io",
				Scheme:   "https",
				Path:     "/token",
				RawQuery: "scope=repository%3Anitroxaddict%2Fvigil%3Apull&service=ghcr.io",
			}
		...
		When("deriving the auth scope from an image name", func() {
			...
			It("should not include vanity hosts\"", func() {
				Expect(getScopeFromImageAuthURL("docker.io/nitroxaddict/vigil")).To(Equal("nitroxaddict/vigil"))
				Expect(getScopeFromImageAuthURL("index.docker.io/nitroxaddict/vigil")).To(Equal("nitroxaddict/vigil"))
			})
			It("should not destroy three segment image names\"", func() {
				Expect(getScopeFromImageAuthURL("piksel/nitroxaddict/vigil")).To(Equal("piksel/nitroxaddict/vigil"))
				Expect(getScopeFromImageAuthURL("ghcr.io/piksel/nitroxaddict/vigil")).To(Equal("piksel/nitroxaddict/vigil"))
			})
			It("should not prepend library/ to image names if they're not on dockerhub", func() {
				...
				Expect(getScopeFromImageAuthURL("ghcr.io/nitroxaddict/vigil")).To(Equal("nitroxaddict/vigil"))
			})
		})
	...
	Describe("GetChallengeURL", func() {
		It("should create a valid challenge url object based on the image ref supplied", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("ghcr.io/nitroxaddict/vigil:latest")
		...
		It("should assume Docker Hub for image refs with no explicit registry", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("nitroxaddict/vigil:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
		It("should use index.docker.io if the image ref specifies docker.io", func() {
			expected := url.URL{Host: "index.docker.io", Scheme: "https", Path: "/v2/"}
			imageRef, _ := ref.ParseNormalizedNamed("docker.io/nitroxaddict/vigil:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
	})
})
